...
export class HighestPageLoadIps extends React.Component
{
	...
	render()
	{
		...
		for (var i = 0; i < this.state.ips.length; i++) {
			...
			if(IPcount > 500)
			{
				statement = <div className="pageload-state warning">
			...
			if(checkIP(IPaddress)){
				statement = <div className="pageload-state okay">
					<div className="icon"><Check/></div>
					<span>No worries, it's just us!</span>
				</div>;
			}		
			
			ipResults.push(<Section key={"ip_"+i} name={"#"+ (1 + i)}>
				<div className="IPaddress">
					<ValueChecker value={IPaddress} />
					<a href={"//whois.domaintools.com/"+IPaddress} target="_blank" rel="noopener noreferrer" className="icon-link" title="IP whois lookup"><Link /></a>
				</div>
				<div className="value">{IPcount} <span className="subtext">page loads</span></div>
				{statement}
			</Section>);
		}
