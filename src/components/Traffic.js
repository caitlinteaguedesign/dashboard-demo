...
export class Traffic extends React.Component
{
	...
	render()
	{
		...
		{
			topReferrer = <a href={this.state.topReferrer} target="_blank" rel="noopener noreferrer" className="text-link truncate">{this.state.topReferrer}</a>
		}
		else {
			...
			<React.Fragment>
				<div className="content">
					<Section name="Page loads">
						<ValueChecker value={this.state.pageloads} threshold={50000} comparison="greaterthan" />
					</Section>
					<Section name="Top referrer">
						{ topReferrer }
						<div className="value">{this.state.referral_count.toLocaleString()}</div> 
					</Section>
				</div>
