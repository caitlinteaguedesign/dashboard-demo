	...
	for (var theme in props.themes) 
	{
		...
		themesList.push(
			<div key={thisTheme.id} className="an-option">
	         <input type="radio" name="theme-selection" id={"radio_"+thisTheme.id} checked={isCurrentTheme} onChange={(e) => props.switchThemes(thisTheme.id)} />
	...
	for (var module in props.modules) 
	{
		...
		modulesList.push(
			<div key={thisModule.id} className="an-option">
	         <input type="checkbox" id={"check_"+thisModule.id} checked={thisModule.visible} onChange={(e) => props.toggleModules(thisModule)}/>
	...
	return (
		<React.Fragment>
		   ...
		   <div className="settings-actions">
		   	<button type="button" className="button" onClick={(e) => props.saveSettings(e)}>Save</button>
		   </div>
		</React.Fragment>	
	)
}